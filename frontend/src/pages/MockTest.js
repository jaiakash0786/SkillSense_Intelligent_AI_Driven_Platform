...
export default function MockTest() {
  ...
  const [codingAnswers, setCodingAnswers] = useState({}); // {idx: "typed text"} for coding questions
  ...
  const fsButtonRef = useRef(null); // native ref for fullscreen button
  const [startTest, setStartTest] = useState(false);
  const [fsFailed, setFsFailed] = useState(false); // true if requestFullscreen was blocked

  // ── Proctoring state ─────────────────────────────────────────────────────
  const [fsWarning, setFsWarning]         = useState(false);
  const [isFullscreen, setIsFullscreen]   = useState(false); // tracks actual fullscreen state
  ...
  const consecutiveMisses = useRef(0); // face-detection: consecutive no-face frames before violation

  // Violation thresholds — auto-submits when exceeded
  // no_face/multiple_faces need 3 consecutive misses × 4 violations = very forgiving
  const LIMITS = { tab_switch: 1, no_face: 2, multiple_faces: 2 };

  // Auto-submit helper (called from logViolation when threshold exceeded)
  const autoSubmitRef = useRef(null); // will point to handleSubmit after it's defined
  ...
  const logViolation = useCallback((type) => {
    ...
    // Count this type of violation
    const typeCount = violations.current.filter(v => v.type === type).length;
    const limit = LIMITS[type];

    const msg =
      type === "tab_switch"       ? `⚠️ Tab switch detected! (${typeCount}/${limit || 1})`
      : type === "no_face"        ? `⚠️ No face detected! (${typeCount}/${limit})`
      : type === "multiple_faces" ? `⚠️ Multiple faces! (${typeCount}/${limit})`
      : type === "copy_attempt"   ? "⚠️ Copy/paste is not allowed!"
      : type === "fullscreen_exit"? "⚠️ Fullscreen exit detected!"
      : "⚠️ Violation detected!";
    setViolationMsg(msg);
    setTimeout(() => setViolationMsg(""), 4000);

    // Auto-submit when threshold hit
  ...
  // ── Fullscreen helpers ────────────────────────────────────────────────────
  // requestFullscreen() is async (returns a Promise). Must be called synchronously
  // inside the click handler (no await before it) to preserve user-gesture activation.
  // We chain .catch() so rejected Promises don't cause unhandled-rejection errors.
  const enterFullscreen = () => {
    const el = document.documentElement;
    if (el.requestFullscreen) {
      el.requestFullscreen().catch(e => console.warn("Fullscreen rejected:", e.message));
  ...
  const startCamera = async () => {
    try {
      ...
      // Don't attach here — video element not in DOM yet (phase still "ready")
      // useEffect below will attach it once phase becomes "test"
    } catch (e) {
  ...
  // Attach stream to <video> element as soon as it is rendered (phase === "test")
  useEffect(() => {
  ...
  const startFaceDetection = async () => {
    ...
    faceInterval.current = setInterval(async () => {
      ...
      try {
        ...
        if (detections.length === 0) {
          ...
          // Need 3 consecutive misses (18s) before logging a violation
          if (consecutiveMisses.current >= 3) {
            logViolation("no_face");
            consecutiveMisses.current = 0; // reset after logging so next violation also needs 3 misses
          }
    ...
    }, 6000); // check every 6 seconds (was 4s)
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        let data;
        if (mockTestIdParam) {
          const res = await fetch(`${API}/mock-test/start-assigned/${mockTestIdParam}`, {
          ...
          const res = await fetch(`${API}/mock-test/start`, {
            ...
            body: JSON.stringify({ resume_id: Number(resumeId), role: roleParam, skill_topic: topic }),
          });
          data = await res.json();
  ...
  // ── Pre-warm camera on ready screen ───────────────────────────────────────
  // Start camera as soon as the ready screen appears so the permission dialog
  // fires NOW — not during beginTest(). That way, when the user clicks
  // "🚀 Begin Test", camera is already streaming and no dialog interrupts fullscreen.
  useEffect(() => {
    if (phase === "ready") {
      startCamera();
    }
  ...
  // Step 1: Begin Test button → go to fullscreen prompt screen
  const beginTest = () => {
    setPhase("fullscreen"); // show dedicated fullscreen prompt
  };

  // Step 2: Native DOM listener on the fullscreen button so requestFullscreen()
  // runs inside a real DOM event handler (Chrome requires this for transient user activation)
  useEffect(() => {
    if (phase !== "fullscreen" || !fsButtonRef.current) return;
    setFsFailed(false); // reset on each fullscreen phase entry
    ...
    const handleNativeClick = () => {
      ...
      // Detect if fullscreen actually activated after 900ms
      setTimeout(() => {
  ...
  // When startTest flips true, actually start face detection + timer + set phase
  useEffect(() => {
    if (!startTest) return;
    setStartTest(false);
    startFaceDetection();
  ...
  }, [startTest]); // eslint-disable-line


  // ── Answer + navigation ───────────────────────────────────────────────────
  const selectAnswer = (letter) => setAnswers(prev => ({ ...prev, [current]: letter }));
  const selectCodingAnswer = (text) => setCodingAnswers(prev => ({ ...prev, [current]: text }));
  ...
  const handleSubmit = async () => {
    ...
    const answersPayload = questions
      ...
      .map(({ q, idx }) => {
        const chosen = answers[idx] || null;  // null = unanswered (never default to A)
        return { question: q.question, chosen, correct: q.answer, is_correct: chosen === q.answer && chosen !== null };
      });

    const codingPayload = questions
      .map((q, idx) => ({ q, idx }))
      .filter(({ q }) => q.type === "coding")
      .map(({ q, idx }) => ({ question: q.question, answer_text: codingAnswers[idx] || "" }));
    ...
    try {
      ...
      if (!res.ok) {
        // FastAPI 422 returns data.detail as an array of pydantic validation errors
        let errMsg;
  ...
  // Wire autoSubmitRef so logViolation can call handleSubmit after threshold
  // (logViolation is memoized so can't directly close over handleSubmit)
  useEffect(() => {
    autoSubmitRef.current = handleSubmit;
  }); // runs every render — keeps ref pointing to latest closure
  ...
  const timerClass = timeLeft <= 120 ? "mt-timer mt-timer-danger"
                   : timeLeft <= 300 ? "mt-timer mt-timer-warn"
                   : "mt-timer";
  ...
  if (phase === "error") {
    return (
      <div className="mt-fullscreen mt-center">
        ...
        <button className="mt-btn mt-btn-secondary" onClick={() => navigate(-1)}>← Go Back</button>
      </div>
    );
  }
      ...
      <div className="mt-fullscreen mt-scrollable">
        <div className="mt-result-card" style={{ maxWidth: 500 }}>
          ...
          <div className="mt-proctor-notice">
            📷 Camera &nbsp;|&nbsp; 👁️ Face monitoring &nbsp;|&nbsp; 🔒 Anti-cheat active
          </div>
  ...
  // ── FULLSCREEN PROMPT phase (between ready and test) ──────────────────────
  if (phase === "fullscreen") {
    return (
      <div className="mt-fullscreen mt-scrollable">
        <div className="mt-result-card" style={{ maxWidth: 500, textAlign: "center" }}>
          ...
          {!fsFailed ? (
            <>
              <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 14, marginBottom: 20 }}>
                Your exam must run in fullscreen mode.<br/>
            ...
            // Fullscreen was blocked by browser — show F11 instruction
            <>
              <div style={{ background: "rgba(248,113,113,0.15)", border: "1px solid #f87171", borderRadius: 12, padding: "16px 20px", marginBottom: 16 }}>
                <p style={{ color: "#f87171", fontWeight: 700, marginBottom: 6 }}>Browser blocked fullscreen</p>
                <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 13, margin: 0 }}>
                  Press <kbd style={{ background: "#334155", padding: "2px 8px", borderRadius: 4, fontFamily: "monospace", fontSize: 15 }}>F11</kbd> on your keyboard to enter fullscreen manually, then continue.
                </p>
              </div>
              ...
              <button
                ...
                onClick={() => { startFaceDetection(); setPhase("test"); setTimeLeft(TIMER_SECONDS); timerRef.current = setInterval(() => setTimeLeft(p => p <= 1 ? (clearInterval(timerRef.current), 0) : p - 1), 1000); }}
              >
                ▶ Start Test Anyway
              </button>
            </>
          ...
          <button
            ...
            style={{ marginTop: 14 }}
            onClick={() => { setFsFailed(false); setPhase("ready"); }}
          >
            ← Back
          </button>
        </div>
  ...
  if (phase === "result") {
    ...
    const tabSwitches  = result.violations?.filter(v => v.type === "tab_switch").length  || 0;
    const noFace       = result.violations?.filter(v => v.type === "no_face").length        || 0;
    const multiFace    = result.violations?.filter(v => v.type === "multiple_faces").length || 0;
    const copyAttempts = result.violations?.filter(v => v.type === "copy_attempt").length   || 0;
      ...
      <div className="mt-fullscreen mt-scrollable">
        <div className="mt-result-card">
          ...
          <div className="mt-result-stats">
            ...
            <div className="mt-stat">
              <span className="mt-stat-val">{result.total_questions - result.correct_answers}</span>
            ...
            <div className="mt-violation-summary">
              ...
              <div className="mt-violation-grid">
                {result.fullscreen_exits > 0 && (
                  <div className="mt-vio-item"><span>🖥️ Fullscreen exits</span><span>{result.fullscreen_exits}</span></div>
                )}
                {tabSwitches > 0 && (
                  <div className="mt-vio-item"><span>🔀 Tab switches</span><span>{tabSwitches}</span></div>
                )}
                {noFace > 0 && (
                  <div className="mt-vio-item"><span>👤 No face detected</span><span>{noFace}×</span></div>
                )}
                {multiFace > 0 && (
                  <div className="mt-vio-item"><span>👥 Multiple faces</span><span>{multiFace}×</span></div>
                )}
                {copyAttempts > 0 && (
                  <div className="mt-vio-item"><span>📋 Copy attempts</span><span>{copyAttempts}</span></div>
                )}
              </div>
            </div>
          )}
          ...
          <div className="mt-result-actions">
            <button className="mt-btn mt-btn-primary" onClick={() => navigate("/progress")}>
              📊 View Progress
            </button>
            <button className="mt-btn mt-btn-secondary" onClick={() => navigate("/student")}>
              🏠 Dashboard
            </button>
          </div>
        </div>
    ...
    <div className="mt-fullscreen" style={{ userSelect: "none" }}>
      ...
      {fsWarning && (
        <div className="mt-violation-banner">
          ...
          <button className="mt-fs-return-btn" onClick={enterFullscreen}>Return to Fullscreen</button>
        </div>
      )}
      ...
      <div className={`mt-camera-pip ${faceStatus !== "ok" ? "mt-camera-alert" : ""}`}>
        {cameraError ? (
          <div className="mt-camera-unavail">📷<br/>No Camera</div>
        ) : (
          <video ref={videoRef} autoPlay muted playsInline className="mt-camera-video" />
        )}
        <div className="mt-face-status">
          ...
          {faceStatus === "multiple_faces" && <span className="mt-face-bad">⚠️ Multi-Face</span>}
        </div>
      ...
      <div className="mt-header">
        ...
        <div className="mt-header-center">
          ...
          <span className="mt-progress-text">{answeredCount} / {questions.length} answered</span>
        </div>
        <div className="mt-header-right">
          ...
          <span className="mt-q-counter">Q {current + 1} of {questions.length}</span>
        </div>
      </div>
      ...
      <div className="mt-body">
        <div className="mt-question-card">
          <div className="mt-question-num">
            ...
            {q?.difficulty && q?.type !== "coding" && (
              <span className={`mt-diff-badge mt-diff-${q.difficulty}`}> {q.difficulty}</span>
            )}
          </div>
          ...
          {(!q?.type || q?.type === "mcq") && (
            <div className="mt-options">
              {q && Object.entries(q.options).map(([letter, text]) => (
                <button
                  key={letter}
                  className={`mt-option ${chosen === letter ? "mt-option-selected" : ""}`}
        ...
        <div className="mt-dots">
          {questions.map((_, i) => (
            <button
              key={i}
              className={`mt-dot ${i === current ? "mt-dot-active" : ""} ${answers[i] ? "mt-dot-answered" : ""}`}
      ...
      <div className="mt-footer">
        <button className="mt-btn mt-btn-secondary" onClick={goPrev} disabled={current === 0}>
          ← Previous
        </button>
        {current < questions.length - 1 ? (
          <button className="mt-btn mt-btn-primary" onClick={goNext}>Next →</button>
        ) : (
          <button className="mt-btn mt-btn-submit" onClick={handleSubmit}>✅ Submit Test</button>
        )}
      </div>
    </div>
  );
