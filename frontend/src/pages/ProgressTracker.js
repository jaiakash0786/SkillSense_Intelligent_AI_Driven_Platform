...
export default function ProgressTracker() {
    ...
    const loadResumeData = useCallback(async (resumeId) => {
        ...
        try {
            ...
            // Build skill progress map from status data
            const progressMap = {};
            ...
            setSkillProgress(progressMap);
                        // The analysis result is flat: { roles, target_role, ats, learning_path }
            // learning_path is { learning_path: [...], role: "..." }
    ...
    const handleToggle = async (skillName, topic) => {
        ...
        try {
            ...
            console.log(`✅ Toggled ${skillName}::${topic}`);
        } catch (err) {
            ...
            {selectedResumeId && !loading && (
                <>
                    ...
                    <div className="progress-content">
                        ...
                        {/* TAB 1: OVERVIEW (Chart + Quick Stats) */}
                        {activeTab === "overview" && (
                            <section className="prog-card chart-card">
                                ...
                                <div className="quick-stats">
                                    <div className="stat-box">
                                        ...
                                        <div className="stat-label">Skills Progress</div>
                                    </div>
                                    <div className="stat-box">
                                        <div className="stat-value">{doneTopics}</div>
                                    ...
                                    <div className="stat-box">
                                        <div className="stat-value">{chartRoles.length}</div>
                                        <div className="stat-label">Roles Tracked</div>
                                    </div>
                                </div>
                                                                ...
                                                                <li key={ti} className={`topic-item${checked ? " done" : ""}`}>
                                                                    ...
                                                                    {checked && (
                                                                        <button
                                                                            ...
                                                                            onClick={() => navigate(`/mock-test?role=${encodeURIComponent(selectedRole || "Candidate")}&topic=${encodeURIComponent(topic)}&resume_id=${selectedResumeId}`)}
                                                                            title="Take a mock test on this topic"
                                                                        >
                                                                            🧪 Test Knowledge
                                                                        </button>
                                                                    )}
                        ...
                        {/* TAB 4: MILESTONES (Badges) */}
                        {activeTab === "milestones" && (
                            <section className="prog-card badges-card">
                                <div className="card-accent" />
                                <h2>🏅 Milestones</h2>
                                <p className="milestones-subtitle">Unlock badges as you complete your learning journey</p>
                                    ...
                                    {BADGE_LEVELS.map(b => {
                                        ...
                                        return (
                                            <div key={b.id} className={`badge-item${earned ? " earned" : " locked"}`}>
                                                ...
                                                <div className="badge-progress">{b.pct}%</div>
